...
iunmap <F4>
inoremap <F4> <ESC>mz:execute JFileHeading()<CR>`z4jA"

" Javadoc file header
function JFuncHeading()
  let s:line=getline(".")
  ...
  for arg in s:args
    let s:temp = substitute(arg, '^\s*\(.\{-}\)\s*$', '\1', '')
    call append(s:newline+s:i, " * @param " . s:temp  . " - ")
    let s:i += 1
  endfor
  unlet s:args
...
iunmap <F3>
inoremap <F3> <ESC>:execute JFuncHeading()<CR>?TODO<CR>Ncw
